/* Program written by Alex Johnson
/  2/10/2019 - CS 403 - Dr. Lusth
/  types.c contains the assignment of the global variables declared in types.h.
/  These variables are all the different types that a lexeme could be.
*/

#include "types.h"

char* OPAREN = "open parenthesis";
char* CPAREN = "close parenthesis";
char* COMMA = "comma";
char* PLUS = "plus";
char* MINUS = "minus";
char* TIMES = "times";
char* DIVIDES = "divides";
char* MODULUS = "modulus";
char* ASSIGN = "assign";
char* EQUALS = "equals";
char* GREATERTHAN = "greater than";
char* LESSTHAN = "less than";
char* DOT = "dot";
char* HASH = "hash";
char* OBRACKET = "open bracket";
char* CBRACKET = "close bracket";
char* AT = "at";
char* INTEGER = "integer";
char* STRING = "string";
char* BOOLEAN = "boolean";
char* FILEPOINTER = "file pointer";
char* VAR = "var";
char* IF = "if";
char* ELSE = "else";
char* WHILE = "while";
char* PRINT = "print";
char* ARRAY = "array";
char* NEWARRAY = "newarray";
char* GETARRAY = "getarray";
char* SETARRAY = "setarray";
char* GETARGCOUNT = "getargcount";
char* GETARG = "getarg";
char* OPENFILE = "openfile";
char* READINTEGER = "readinteger";
char* ATFILEEND = "atfileend";
char* CLOSEFILE = "closefile";
char* DEF = "def";
char* START = "start";
char* STOP = "stop";
char* AND = "and";
char* OR = "or";
char* NOT = "not";
char* NEGATIVE = "negative";
char* LAMBDA = "lambda";
char* ID = "id";
char* VAL = "val";
char* ERROR = "ERROR";
char* END_OF_INPUT = "EOF";

char* ENV = "environment";
char* TABLE = "table";

char* ACCESS = "access";
char* CALL = "call";
char* IDLIST = "id list";
char* EXPLIST = "expression list";
char* PROGRAM = "program";
char* VARDEF = "variable definition";
char* VARASSIGN = "variable assignment";
char* FUNCDEF = "function definition";
char* JUNK = "junk";
char* BLOCK = "block";
char* STATEMENTLIST = "statement list";
char* IFSTATEMENT = "if statement";
char* ELSESTATEMENT = "else statement";
char* WHILELOOP = "while loop";

char* CLOSURE = "closure";
char* JOIN = "join";
